...
from BnuAutoLogin.configs.settings import STATIC_DIR
...
class Login:
    ...
    def __init__(self, username, password):
        ...
        # 加载js上下文环境
        self.verify_ctx = self._get_js_context(STATIC_DIR / 'verify.js')
    ...
    def login(self):
        ...
        login_response = self.requests.get(LOGIN_URL,
                                           params=data,
                                           headers=HEADER)
        self._parse_response_data(login_response)
    ...
    def logout(self):
        ...
        sign = self.verify_ctx.call(
            'sha1', t+self.username+self.ip+str(unbind)+t)
        data = {
            'callback': self.ccn,
            'ip': self.ip,
            'time': t,
    ...
    def _get_password_hmd5(self, token: str) -> str:
        ...
        Returns:
            str: password_hmd5
        '''
        self.hmd5 = self.verify_ctx.call('md5', self.password, token)
    ...
    def _get_info(self, token: str):
        ...
        # i = self.verify_ctx.call('xEncode', i_dv.replace(' ', ''), token)
        i = self.verify_ctx.call('xEncode', i_dv.replace(' ', ''), token).encode(
            'gbk').decode('utf-8')
        info = "{SRBX1}" + self.verify_ctx.call('encode', i)
        return info
    ...
    def _get_chkstr(self, token, info):
        ...
        chkstr += token + info
        chkstr = self.verify_ctx.call('sha1', chkstr)
        return chkstr
