$(function () {
    ...
    $("#self-service").click(function () {
        ...
        if (typeof username != "undefined" && username != "") {
            ...
            if (data != "") {
                window.open((portal.ServiceIP || location.hostname) + ":8800/site/sso?data=" + data);
                return;
            }
            window.open((portal.ServiceIP || location.hostname) + ":8800");
            return;
        }
        window.open((portal.ServiceIP || location.hostname) + ":8800");
    });
    ...
    function IpAlreadyRetryAuth(params) {
        ...
        $.DM(host, logoutParms, function (data) {
            if (data.error == "ok") {
    ...
    $("#logout-dm").click(function () {
        ...
        $.DM(host, params, function (data) {
            if (data.error == "ok") {
                //Show DM Logout OK!
                showErrorMessage("LogoutOK", true, true);
            } else {
                if (data.message == "NoResponseDataError") {
                    showLog(username);
                    return;
    ...
    var createQrcode = (function () {
        ...
        $('#out-qrcode1 canvas').after("<p class='intro'>第二步:小程序中认证后 <br> 点击右上角扫这里</p>")
